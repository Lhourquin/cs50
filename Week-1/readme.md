...
1 error generated.
```
Placing the semicolon back  in the correct position and running `gcc hello.c -o hello` again, the erros go away.
* Notice also the special symbol `\n` in your code. Try removing those characters and `making` your program again by executiing `gcc hello.c -o hello`. Typing `./hello` in the terminal:
```
❯ ./hello 
Helllo, world% 
```
...
* Recall that in Sratch, we had the ability to ask the user "What's your name ?" and say "Hello" with that name appended to it.
...
* `answer` is a special holding place we call a *variable*. `answer` is of type `string` and can hold any string whitin it. There are many *data types* such as `int`, `bool`, `char`, and many others.
...
* `printf` allows for many format codes. Here is a noncomprhensive list of ones you may utilize in this course:
```
%c
%f
%i
